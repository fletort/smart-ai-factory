...
3. **Scope Evaluation**: Once the discussion is mature, evaluate the scope:
   - If **SMALL/MICRO** (< 4 hours estimated work): Present a choice between:
      - **Case 1** (Run `/smart-plan` to append a high-level issue placeholder to the active roadmap using the
        chat context.) Use when tracking is needed but local specification documentation is overkill.
